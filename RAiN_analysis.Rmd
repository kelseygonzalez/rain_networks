...
files_filtered <- filter(files, grepl("rds", row.names(files)))
...
# Generate data
#map(clean_data$important_people, group_by, important_alter_type) %>% map_dbl(~nrow(distinct(., important_alter_name)))

persNet_descriptives <- clean_data %>% 
  mutate(nTiesAct = map_dbl(alter_data, ~nrow(distinct(., alter_name))),
         nTiesImpFamily = ,
         nTiesImpNonFamily)

# Number of ties from activation
nTiesAct_descrPlot <- map(clean_data$alter_data, ~nrow(distinct(., alter_name))) %>% 
  map_dfr(as_tibble, .name_repair = "universal") %>%
  ggplot(aes(value)) +
    geom_density() +
    main_theme_noMargin_smallFont

# Number of important family
# clean_data$important_people %> map_dbl(important_alter_type)
# clean_data$important_people[[1]] %>% count(important_alter_type) %>% .[1,2]

nTiesImpFamily_descrPlot <- map(clean_data$important_people, group_by, important_alter_type) %>% 
  map_dbl(~nrow(distinct(., important_alter_name))) %>%
  ggplot(aes(value)) +
    geom_density() +
    main_theme_noMargin_smallFont

# Number of important non-family
...
nActivation_descrPlot <- map(clean_data$alter_data, ~nrow(distinct(., act_instance))) %>% 
  map_dfr(as_tibble, .name_repair = "universal") %>%
  ggplot(aes(value)) +
    geom_density() +
    main_theme_noMargin_smallFont
